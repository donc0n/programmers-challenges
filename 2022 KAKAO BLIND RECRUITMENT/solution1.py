...
def solution(id_list, report, k): # O(report)
    ...
    for r in set(report):
        ...
        count[reported] += 1
    is_reported = { key: 1 if value >= k else 0 for key, value in count.items() }
    for id in id_list:
        answer.append(sum(map(lambda x: is_reported[x], report_dict[id])))
    return answer